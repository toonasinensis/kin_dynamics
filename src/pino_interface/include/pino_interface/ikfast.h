...
class IkSingleDOFSolutionBase
{
  public:
    ...
    unsigned char maxsolutions = 0; ///< max possible indices, 0 if controlled by
                                    ///< free index or a free joint itself
    std::array<unsigned char, IKSINGLEDOFSOLUTIONBASE_INDICES_SIZE>
        indices; ///< unique index of the solution used to keep track on what part
                 ///< it came from. sometimes a solution can be repeated for
                 ///< different indices. store at least another repeated root

    virtual void Print() const
    {
        std::cout << "(" << ((jointtype == 0x01) ? "R" : "P") << ", " << (int)freeind << "), ("
                  << foffset << ", " << fmul << "), " << (unsigned int)maxsolutions << " (";
  ...
  public:
    IkFastFunctions()
        : _ComputeIk(NULL), _ComputeIk2(NULL), _ComputeFk(NULL), _GetNumFreeParameters(NULL),
          _GetFreeIndices(NULL), _GetNumJoints(NULL), _GetIkRealSize(NULL), _GetIkFastVersion(NULL),
          _GetIkType(NULL), _GetKinematicsHash(NULL)
    {
    }
  ...
  public:
    ...
    virtual void Validate() const
    {
        for (size_t i = 0; i < _vbasesol.size(); ++i)
        {
            ...
            {
                throw std::runtime_error("max solutions for joint " + std::to_string(i) +
                                         "not initialized");
            }
            ...
            {
                throw std::runtime_error("foffset for joint " + std::to_string(i) +
                                         " was not finite");
            }
        }
    }
...
class IkSolutionList : public IkSolutionListBase<T>
{
  public:
    ...
    {
        _listsolutions.clear();
        std::move(std::make_move_iterator(vecsols.begin()), std::make_move_iterator(vecsols.end()),
...
///
/// \param freejoint  Index of the  free joint jy in SolutionArray =
/// std::array<T, N>. \param mimicjoint Index of the mimic joint jx in
/// SolutionArray. \param solutionindex Index of the IK solution in
/// std::vector<SolutionArray>.
///        This is NOT the solution index associated with each joint in one IK
///        solution.
/// \param bxpy If true, then jx + jy is the constant jxpy; otherwise jx - jy is
/// the constant jxmy.
///        We store this constant in the foffset field of jx, and store 0 in
///        that of jy.
///
...
struct AlignedSolution
{
    ...
    AlignedSolution(uint32_t freejoint_in, uint32_t mimicjoint_in, uint32_t solutionindex_in,
                    bool bxpy_in)
    {
...
{
    IndicesCompare(const std::vector<ikfast::IkSolution<T>> &vecsols,
                   const std::array<uint32_t, N> &jointorder)
        : _vecsols(vecsols), _jointorder(jointorder)
    {
    }

    bool operator()(uint32_t inda, uint32_t indb)
    {
        const ikfast::IkSolution<T> &sola = _vecsols[inda], &solb = _vecsols[indb];
        for (size_t i = 0; i < _jointorder.size(); ++i)
        {
            const T x = sola.get(_jointorder[i]).foffset, y = solb.get(_jointorder[i]).foffset;
            if (x != y)
            {
                return x < y;
            }
                           ...
                           const std::array<uint32_t, N> &jointorder)
{
    ...
    for (uint32_t varindex = 0; varindex < N; varindex++)
    {
        const uint32_t jointindex = jointorder[varindex], oldsi = count[varindex],
